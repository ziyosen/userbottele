...
async def sticker_handler(client, message: Message):
    ...
    sticker_webp = f"{TEMP_DIR}/temp_{message.id}.webp"
...
async def bulge_handler(client, message: Message):
    ...
    status = await message.edit(info(f"Applying Bulge (str={strength})..."))
    download_path = await client.download_media(message.reply_to_message)
    bulge_webp = f"{TEMP_DIR}/bulge_{message.id}.webp"

    try:
        # Menjalankan proses berat di thread terpisah agar bot tidak beku
        def process_bulge():
            img = Image.open(download_path).convert("RGBA")
            img.thumbnail((512, 512), Image.Resampling.LANCZOS)
            w, h = img.size
                ...
                for x in range(w):
                    ...
                    if 0 < r < max_radius:
                        ...
                        if 0 <= sx < w-1 and 0 <= sy < h-1:
                            ...
                            c00, c10 = orig[x0, y0], orig[min(x0+1, w-1), y0]
                            c01, c11 = orig[x0, min(y0+1, h-1)], orig[min(x0+1, w-1), min(y0+1, h-1)]
        ...
        # Eksekusi fungsi berat tanpa memblokir event loop
        await asyncio.get_event_loop().run_in_executor(None, process_bulge)
